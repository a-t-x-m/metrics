...
var _crypto = _interopRequireDefault(crypto);
...
var Metrics = /** @class */ (function () {
    ...
    Metrics.prototype.commandListener = function () {
        ...
        filteredCommands.map(function (command) {
            log(_this.title + ": Adding event listener for command:", command);
    ...
    Metrics.prototype.getMacAddress = function () {
        var macAddress = mac(function (error, data) { return error
            ? null
            : data.toString(); });
        if (macAddress !== null)
            log(this.title + ": Detected MAC address '" + macAddress + "'");
        return macAddress;
    };
